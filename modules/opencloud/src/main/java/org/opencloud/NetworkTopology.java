...
import org.opencloud.network.DelayMatrix_Float;
import org.opencloud.network.GraphReaderBrite;
import org.opencloud.network.TopologicalGraph;
import org.opencloud.network.TopologicalLink;
import org.opencloud.network.TopologicalNode;

/**
 * Implements the network layer in OpenCloud. It reads a file in the <a href="http://www.cs.bu.edu/brite/user_manual/node29.html">BRITE format</a>,
 ...
 * in network traffic of OpenCloud.
 * <p/>
 ...
 * Nevertheless, each OpenCloud entity must be mapped to one (and only one) BRITE node to allow
...
public class NetworkTopology {
	...
	/**
         * The map between OpenCloud entities and BRITE entities.
         * Each key is a OpenCloud entity ID and each value the corresponding
         * BRITE entity ID.
         */
	 ...
	 * Adds a new link in the network topology.
         * The OpenCloud entities that represent the source and destination of the link
         * will be mapped to BRITE entities.
	 * 
	 * @param srcId ID of the OpenCloud entity that represents the link's source node
	 * @param destId ID of the OpenCloud entity that represents the link's destination node
	...
	/**
	 * Maps a OpenCloud entity to a BRITE node in the network topology.
	 * 
	 * @param entityID ID of the entity being mapped
	 * @param briteID ID of the BRITE node that corresponds to the OpenCloud entity
	 * @pre entityID >= 0
	 * @pre briteID >= 0
	 * @post $none
	 */
	public static void mapNode(int entityID, int briteID) {
		if (networkEnabled) {
			try {
				// this OpenCloud entity was already mapped?
				if (!map.containsKey(entityID)) {
					if (!map.containsValue(briteID)) { // this BRITE node was already mapped?
						map.put(entityID, briteID);
					} else {
						Log.printlnConcat("Error in network mapping. BRITE node ", briteID, " already in use.");
					}
				} else {
					Log.printlnConcat("Error in network mapping. OpenCloud entity ", entityID,
							" already mapped.");
				}
			} catch (Exception e) {
				Log.printlnConcat("Error in network mapping. OpenCloud node ", entityID,
	...
	/**
	 * Unmaps a previously mapped OpenCloud entity to a BRITE node in the network topology.
	 * 
	 * @param entityID ID of the entity being unmapped
	 * @pre entityID >= 0
	 * @post $none
	 */
	public static void unmapNode(int entityID) {
		if (networkEnabled) {
			try {
				map.remove(entityID);
			} catch (Exception e) {
				Log.printlnConcat("Error in network unmapping. OpenCloud node: ", entityID);
			}
		}
	}
