...
//        Log.printLine("Power data center is Updating the cloudlet processing");
        ...
        if (currentTime > getLastProcessTime()) {
            ...
            if (!isDisableVmMigrations()) {
                ...
                Log.printlnConcat(OpenCloud.clock(), ": The number of Migrations is:  ", getVmMigrationCount() - previousMigrationCount);
//                String[] msg={Double.toString (OpenCloud.clock()), Integer.toString (getVmMigrationCount() - previousMigrationCount)  } ;                   // <--declared statement
//                try {
//                    getVmMigrationWriter().writeTofile(msg);
//                } catch (IOException e) {
//                    e.printStackTrace();
//                }
//                Log.printConcatLine(OpenCloud.clock(),": The total number of Migrations is:  ",getVmMigrationCount());
            }
...
//        Log.printLine("Power data center is Updating CloudetProcessingWithoutSchedulingFutureEvents");
        if (OpenCloud.clock() > getLastProcessTime()) {
...
//        Log.printLine("Power data center is Updating CloudetProcessingWithoutSchedulingFutureEventsForce");
    ...
    /*
     * (non-Javadoc)
     * @see org.opencloud.Datacenter#processVmMigrate(org.opencloud.core.SimEvent,
     * boolean)
     */
    @Override
    protected void processVmMigrate(SimEvent ev, boolean ack) {
        ...
        if (event == null || event.eventTime() > OpenCloud.clock()) {
...
//        SimEvent event = OpenCloud.findFirstDeferred(getId(), new PredicateType(containerSimTags.CONTAINER_MIGRATE));
//        if (event == null || event.eventTime() > OpenCloud.clock()) {
//            updateCloudetProcessingWithoutSchedulingFutureEventsForce();
//        }
//    }
    /*
     * (non-Javadoc)
     * @see opencloud.Datacenter#processCloudletSubmit(opencloud.core.SimEvent, boolean)
     */
    @Override
    protected void processCloudletSubmit(SimEvent ev, boolean ack) {
//        Log.printLine("Power data center is processing cloudlet submit");
        super.processCloudletSubmit(ev, ack);
//        Log.printLine("Power Data Center : Set Cloudlet Submited to " + OpenCloud.clock());
        setCloudletSubmitted(OpenCloud.clock());
    }
