...
public class ContainerDatacenterBroker extends DatacenterBroker {
 ...
 /**
    ...
    public void processEvent(SimEvent ev) {
        ...
        if (tag == ContainerOpenCloudTags.VM_NEW_CREATE) {
            processNewVmCreate((GuestMapping)ev.getData());
        } else if (tag == ContainerOpenCloudTags.CONTAINER_CREATE_ACK) {
    ...
    public void processContainerCreate(SimEvent ev) {
        ...
        if (result == OpenCloudTags.TRUE) {
            ...
            Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": The Container #", containerId,
            ...
            Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Failed Creation of Container #", containerId);
        }
    ...
    protected void processVmCreateAck(SimEvent ev) {
        ...
        if (result == OpenCloudTags.TRUE) {
            ...
            Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Creation of VM #", vmId,
...
//                    Log.printLine(OpenCloud.clock() + ": " + getName()
...
//                    if (!Log.isDisabled()) {
//                        Log.printConcatLine(OpenCloud.clock(), ": ", getName(), ": Postponing execution of cloudlet ",
...
//                Log.printConcatLine(OpenCloud.clock(), ": ", getName(), ": Sending cloudlet ",
...
//                    double newmips = maxUsage * container.getMips();
        ...
        sendNow(getDatacenterIdsList().get(0), ContainerOpenCloudTags.CONTAINER_SUBMIT, successfullySubmitted);

//        List<Container> successfullySubmitted = new ArrayList<>();
//        for (Container container : getContainerList()) {
//
//            if (!Log.isDisabled()) {
//                Log.printConcatLine(OpenCloud.clock(), ": ", getName(), ": Sending container ",
