...
package org.opencloud.container.core;

import org.opencloud.*;
import org.opencloud.VmAllocationPolicy.GuestMapping;
import org.opencloud.container.utils.CustomCSVWriter;
...
//                                            create a new event for this. or overright the vm create
                                    ...
                                    send(
                                            ...
                                            , ContainerOpenCloudTags.CONTAINER_MIGRATE,
                                            migrate);
...
//                                    it would be 100.4
                ...
                Log.printlnConcat(OpenCloud.clock(), ": The Number Container of Migrations is:  ", getContainerMigrationCount() - previousContainerMigrationCount);
                Log.printlnConcat(OpenCloud.clock(), ": The Number of VM Migrations is:  ", getVmMigrationCount() - previousVmMigrationCount);
                String[] vmMig = {Double.toString(OpenCloud.clock()), Integer.toString(getVmMigrationCount() - previousVmMigrationCount)};                   // <--declared statement
                String[] msg = {Double.toString(OpenCloud.clock()), Integer.toString(getContainerMigrationCount() - previousContainerMigrationCount)};                   // <--declared statement
                try {
                ...
                String[] msg1 = {Double.toString(OpenCloud.clock()), Integer.toString(numberOfNewVms)};                   // <--declared statement
                try {
...
//                containerVm.addMigratingInContainer((Container) map.container());
            ...
            if (result) {
                ...
                send(2, OpenCloud.getMinTimeBetweenEvents(), ContainerOpenCloudTags.VM_NEW_CREATE, data);
                Log.println(String.format("%s VM ID #%d is created on Host #%d", OpenCloud.clock(), containerVm.getId(), host.getId()));
                ...
                containerVm.updateCloudletsProcessing(OpenCloud.clock(), getVmAllocationPolicy().getHost(containerVm).getGuestScheduler()
