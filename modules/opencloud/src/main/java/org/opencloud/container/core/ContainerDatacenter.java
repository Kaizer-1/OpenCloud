...
public class ContainerDatacenter extends Datacenter {
    ...
    public void processEvent(SimEvent ev) {
        ...
        if (tag == ContainerOpenCloudTags.CONTAINER_SUBMIT) {
            processContainerSubmit(ev, true);
        } else if (tag == ContainerOpenCloudTags.CONTAINER_MIGRATE) {
    ...
    public void processContainerSubmit(SimEvent ev, boolean ack) {
        ...
        for (Container container : containerList) {
            ...
            if (ack) {
                ...
                send(ev.getSourceId(), OpenCloud.getMinTimeBetweenEvents(), ContainerOpenCloudTags.CONTAINER_CREATE_ACK, data);
            }
            if (result) {
                ...
                container.updateCloudletsProcessing(OpenCloud.clock(), getContainerAllocationPolicy().getHost(container).getGuestScheduler().getAllocatedMipsForGuest(container));
            } else {
    ...
    protected void processContainerMigrate(SimEvent ev, boolean ack) {
        ...
        if (ack) {
            ...
            sendNow(ev.getSourceId(), ContainerOpenCloudTags.CONTAINER_CREATE_ACK, data);
        }
