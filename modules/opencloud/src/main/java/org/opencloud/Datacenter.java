...
public class Datacenter extends SimEntity {
	...
	/** The regional Cloud Information Service (CIS) name. 
         * @see org.opencloud.core.CloudInformationService
         */
	 ...
	 *  <ul>
	 *    <li>creating this entity before initializing OpenCloud package
	...
	protected void processDataDelete(SimEvent ev, boolean ack) {
		...
		OpenCloudTags tag = CloudActionTags.BLANK;
	...
	protected void processDataAdd(SimEvent ev, boolean ack) {
		 ...
		 * " from " + OpenCloud.getEntityName(sentFrom));
		 */
	 ...
	 * Process non-default received events that aren't processed by
         * the {@link #processEvent(org.opencloud.core.SimEvent)} method.
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		...
		if (ack) {
			...
			data[0] = getId();
			data[1] = guest.getId();
			data[2] = result ? OpenCloudTags.TRUE : OpenCloudTags.FALSE;
			send(guest.getUserId(), OpenCloud.getMinTimeBetweenEvents(), CloudActionTags.VM_CREATE_ACK, data);
		}
		...
		if (result) {
			...
			guest.updateCloudletsProcessing(OpenCloud.clock(), getVmAllocationPolicy().getHost(guest).getGuestScheduler()
					.getAllocatedMipsForGuest(guest));
		} else {
			Log.printlnConcat(OpenCloud.clock(), ": Datacenter.guestAllocator: Couldn't find a host for ", guest.getClassName(), " #", guest.getId());
		}
	}
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if (cl.isFinished()) {
				String name = OpenCloud.getEntityName(cl.getUserId());
				...
				// Hence, this might cause OpenCloud to be hanged since waiting
				// for this Cloudlet back.
				if (ack) {
					int[] data = new int[3];
					data[0] = getId();
					data[1] = cl.getCloudletId();
					data[2] = OpenCloudTags.FALSE;
			...
			/*else {
				Log.printlnConcat(OpenCloud.clock(), ": [",getName(), "]: Warning - ", cl.getClass().getSimpleName()," #", cl.getCloudletId(),
	...
	protected void processCloudletResume(int cloudletId, int userId, int vmId, boolean ack) {
		...
		if (ack) {
			...
			data[0] = getId();
			data[1] = cloudletId;
			data[2] = status ? OpenCloudTags.TRUE : OpenCloudTags.FALSE;
	...
	protected void updateCloudletProcessing() {
		...
		if (OpenCloud.clock() < 0.111 || OpenCloud.clock() >= getLastProcessTime() + OpenCloud.getMinTimeBetweenEvents()) {
			double smallerTime = Double.MAX_VALUE;
			for (HostEntity host : getVmAllocationPolicy().getHostList()) {
				// inform VMs to update processing
				double time = host.updateCloudletsProcessing(OpenCloud.clock());
			...
			if (smallerTime < OpenCloud.clock() + OpenCloud.getMinTimeBetweenEvents() + 0.01) {
				smallerTime = OpenCloud.clock() + OpenCloud.getMinTimeBetweenEvents() + 0.01;
			}
			if (smallerTime != Double.MAX_VALUE) {
				schedule(getId(), (smallerTime - OpenCloud.clock()), CloudActionTags.VM_DATACENTER_EVENT);
			}
			setLastProcessTime(OpenCloud.clock());
		}
	}
	...
	public void startEntity() {
		...
		int gisID = OpenCloud.getEntityId(regionalCisName);
		if (gisID == -1) {
			gisID = OpenCloud.getCloudInfoServiceEntityId();
		}
