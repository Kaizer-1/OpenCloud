...
import org.opencloud.lists.CloudletList;
...
public class DatacenterBroker extends SimEntity {
	...
	protected void processResourceCharacteristicsRequest(SimEvent ev) {
		setDatacenterIdsList(OpenCloud.getCloudResourceList());
		setDatacenterCharacteristicsList(new HashMap<>());

		Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Cloud Resource List received with ",
	...
	protected void processVmCreateAck(SimEvent ev) {
		...
		if (result == OpenCloudTags.TRUE) {
			...
			Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": ", guest.getClassName(), " #", vmId,
					" has been created in Datacenter #", datacenterId, ", ", guest.getHost().getClassName(), " #",
					guest.getHost().getId());
		} else {
			Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Creation of ", guest.getClassName(), " #", vmId,
		...
		if (getGuestsCreatedList().size() == getGuestList().size() - getVmsDestroyed()) {
			...
			if (getVmsRequested() == getVmsAcks()) {
				...
				if (!getGuestsCreatedList().isEmpty()) { // if some vm were created
					...
					Log.printlnConcat(OpenCloud.clock(), ": ", getName(),
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": ", cloudlet.getClass().getSimpleName(), " #", cloudlet.getCloudletId(),
				" return received");
		Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": The number of finished Cloudlets is:", getCloudletReceivedList().size());
		cloudletsSubmitted--;
		if (getCloudletList().isEmpty() && cloudletsSubmitted == 0) { // all cloudlets executed
			Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": All Cloudlets executed. Finishing...");
			clearDatacenters();
			finishExecution();
	...
	protected void createVmsInDatacenter(int datacenterId) {
		...
		int requestedVms = 0;
		String datacenterName = OpenCloud.getEntityName(datacenterId);
		for (GuestEntity vm : getGuestList()) {
			if (!getVmsToDatacentersMap().containsKey(vm.getId())) {
				Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Trying to Create ", vm.getClassName(),
		...
		for (Cloudlet cloudlet : getCloudletList()) {
			...
			if (cloudlet.getGuestId() == -1) {
				...
				if (vm == null) { // vm was not created
					...
					if(!Log.isDisabled()) {
						if (vm != null) {
							Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Postponing execution of cloudlet ",
			...
			if (!Log.isDisabled()) {
				Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Sending ", cloudlet.getClass().getSimpleName(),
	...
	protected void clearDatacenters() {
		for (GuestEntity vm : getGuestsCreatedList()) {
			Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Destroying ", vm.getClassName(), " #", vm.getId());
