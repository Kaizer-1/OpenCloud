...
import org.opencloud.provisioners.BwProvisioner;
import org.opencloud.provisioners.RamProvisioner;
 ...
 * <ul>
 * <li><a href="http://dx.doi.org/10.1109/UCC.2011.24">Saurabh Kumar Garg and Rajkumar Buyya, NetworkOpenCloud: Modelling Parallel Applications in Cloud
    ...
    public void sendPackets() {
		...
		for (Integer guestId : sendPktExternally.keySet()) {
			...
			for (NetworkPacket npkt : sendPktExternally.get(guestId)) {
				...
				OpenCloud.send(getDatacenter().getId(), sw.getId(), delay, CloudActionTags.NETWORK_PKT_UP, npkt);
			}
		}
		...
		if (flag) {
			for (GuestEntity guest : super.getGuestList()) {
				guest.updateCloudletsProcessing(OpenCloud.clock(), getGuestScheduler().getAllocatedMipsForGuest(guest));
			}
		}
	}
