...
import org.opencloud.web.*;
import org.opencloud.web.workload.IWorkloadGenerator;
...
public class WebBroker extends MonitoringBrokerEX {
    ...
    /**
     * By default OpenCloud's brokers use all available datacenters. So we need
            ...
            for (WebSession session : webSessions) {
                ...
                if (session.getAppVmId() == null || session.getDbBalancer() == null) {
                    ...
                    if (session.areVirtualMachinesReady()) {
                        session.notifyOfTime(OpenCloud.clock());
                    } else {
                        ...
                        session.notifyOfTime(OpenCloud.clock() + stepPeriod);
                    }
     ...
     * 
     * @see
     * org.opencloud.DatacenterBroker#processOtherEvent(org.opencloud
     * .opencloud.plus.SimEvent)
     */
     ...
     * 
     * @see
     * org.opencloud.DatacenterBroker#processCloudletReturn(org.opencloud
     * .opencloud.plus.SimEvent)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see opencloud.plus.SimEntity#startEntity()
     */
    @Override
    ...
    protected void processResourceCharacteristicsRequest(final SimEvent ev) {
        setDatacenterIdsList(ev.getData() == null ? OpenCloud.getCloudResourceList() : (List<Integer>) ev.getData());
