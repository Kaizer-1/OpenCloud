...
import org.opencloud.EX.billing.IVmBillingPolicy;
import org.opencloud.EX.util.CustomLog;
import org.opencloud.EX.vm.VmStatus;
import org.opencloud.EX.vm.VmEX;
import org.opencloud.lists.VmList;
...
public class DatacenterBrokerEX extends DatacenterBroker {
    ...
    public void processEvent(SimEvent ev) {
        if (!started) {
            ...
            if (getLifeLength() > 0) {
                send(getId(), getLifeLength(), OpenCloudEXTags.BROKER_DESTROY_ITSELF_NOW, null);
            }
        }
    ...
    protected void processCloudletReturn(SimEvent ev) {
        ...
        if (getLifeLength() <= 0) {
            ...
            Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": ", cloudlet.getClass().getSimpleName()," #", cloudlet.getCloudletId(), " return received");
            cloudletsSubmitted--;
     ...
     * @param delay
     */
    public void presetEvent(final int id, final OpenCloudTags tag, final Object data, final double delay) {
    ...
    public void createVmsAfter(List<? extends Vm> vms, double delay) {
        if (started) {
            send(getId(), delay, OpenCloudEXTags.BROKER_SUBMIT_VMS_NOW, vms);
        } else {
            presetEvent(getId(), OpenCloudEXTags.BROKER_SUBMIT_VMS_NOW, vms, delay);
        }
    }
    ...
    public void destroyVMsAfter(final List<? extends Vm> vms, double delay) {
        if (started) {
            send(getId(), delay, OpenCloudEXTags.BROKER_DESTROY_VMS_NOW, vms);
        } else {
            presetEvent(getId(), OpenCloudEXTags.BROKER_DESTROY_VMS_NOW, vms, delay);
        }
    }
    ...
    public void submitCloudletList(List<? extends Cloudlet> cloudlets, double delay) {
        if (started) {
            send(getId(), delay, OpenCloudEXTags.BROKER_CLOUDLETS_NOW, cloudlets);
        } else {
            presetEvent(getId(), OpenCloudEXTags.BROKER_CLOUDLETS_NOW, cloudlets, delay);
        }
    }
    ...
    protected void processOtherEvent(SimEvent ev) {
        ...
        if (tag == CloudActionTags.VM_DESTROY_ACK) {
            processVMDestroy(ev);
        } else if (tag == OpenCloudEXTags.BROKER_DESTROY_VMS_NOW) {
            destroyVMList((List<Vm>) ev.getData());
        } else if (tag == OpenCloudEXTags.BROKER_SUBMIT_VMS_NOW) {
        ...
        } else if (tag == OpenCloudEXTags.BROKER_CLOUDLETS_NOW) {
            submitCloudletList((List<Cloudlet>) ev.getData());
            submitCloudlets();
        } else if (tag == OpenCloudEXTags.BROKER_DESTROY_ITSELF_NOW) {
            closeDownBroker();
        } else {
    ...
    private void processVMDestroy(SimEvent ev) {
        ...
        if (result == OpenCloudTags.TRUE) {
            ...
            Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": VM #", vmId,
                    " has been destroyed in Datacenter #", datacenterId);
        } else {
            // Use the standard log for consistency ....
            Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Desctuction of VM #", vmId,
    ...
    public void destroyVMList(final List<? extends Vm> vms) {
        ...
        for (final GuestEntity vm : vms) {
            ...
            vm.getHost().updateCloudletsProcessing(OpenCloud.clock());

            int datacenterId = vm.getHost().getDatacenter().getId();
            String datacenterName = vm.getHost().getDatacenter().getName();

            Log.printlnConcat(OpenCloud.clock(), ": ", getName(), ": Trying to Destroy VM #", vm.getId(), " in ",
                    datacenterName);
    ...
    /**
     * OpenCloud does not execute events that are fired before the simulation has
    ...
    protected static class PresetEvent {
        final int id;
        final OpenCloudTags tag;
        final Object data;
        final double delay;

        public PresetEvent(final int id, final OpenCloudTags tag, final Object data, final double delay) {
            super();
            this.id = id;
            this.tag = tag;
            this.data = data;
