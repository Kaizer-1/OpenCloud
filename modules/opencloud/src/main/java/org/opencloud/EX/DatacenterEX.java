...
import org.opencloud.EX.delay.ConstantVmBootDelay;
import org.opencloud.EX.delay.IVmBootDelayDistribution;
...
public class DatacenterEX extends Datacenter {
    ...
    protected void processOtherEvent(final SimEvent ev) {
        if (ev.getTag() == OpenCloudEXTags.DATACENTER_BOOT_VM_TAG) {
    ...
    protected void processVmCreate(final SimEvent ev, final boolean ack) {
        ...
        if (result) {
            send(getId(), delay, OpenCloudEXTags.DATACENTER_BOOT_VM_TAG, vm);

            getVmList().add(vm);

            // May not be instantiated yet

            vm.updateCloudletsProcessing(OpenCloud.clock(), getVmAllocationPolicy().getHost(vm).getGuestScheduler()
