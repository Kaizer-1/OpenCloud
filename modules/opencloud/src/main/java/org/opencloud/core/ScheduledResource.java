...
public class ScheduledResource extends SimEntity {
    ...
    public void startEntity() {
        this.lastClock = OpenCloud.clock();
    }

    public void updateAmounts() {
        double now = OpenCloud.clock();
    ...
    public void processEvent(SimEvent ev) {
        ...
        Log.printlnConcat(OpenCloud.clock(), ": ", getName(), " processEvent(), tag=", tag);
        if (tag == CloudActionTags.NETWORK_PKT_FORWARD) {
            updateAmounts();
            ResourceUse ru = amountsLeft.get(0);
            amountsLeft.remove(ru);
            OpenCloud.send(ru.src, ru.dst, 0.0, ru.tag, ru.data);
            scheduleNext();
        } else if (tag == CloudActionTags.BLANK) {
            ResourceUse ru = (ResourceUse) ev.getData();
            Log.printlnConcat(OpenCloud.clock(), ": ", getName(), " processEvent(), ru.amountLeft=", ru.amountLeft);
            enqueue(ru);
        }
    }
}
