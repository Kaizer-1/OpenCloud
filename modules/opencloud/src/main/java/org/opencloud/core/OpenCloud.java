...
import org.opencloud.container.core.Container;
import org.opencloud.core.predicates.Predicate;
import org.opencloud.core.predicates.PredicateAny;
import org.opencloud.core.predicates.PredicateNone;
...
public class OpenCloud {
	...
	/** The Constant OPENCLOUD_VERSION_STRING. */
	private static final String OPENCLOUD_VERSION_STRING = "7.0";

	/** The id of CIS entity. */
	private static int cisId = -1;

	/** The id of OpenCloudShutdown entity. */
	 ...
	 * @throws Exception This happens when creating this entity before initialising OpenCloud package
	...
	private static void initCommonVariable(Calendar _calendar, boolean _traceFlag, int numUser)
		...
		// creates a OpenCloudShutdown object
		OpenCloudShutdown shutdown = new OpenCloudShutdown("OpenCloudShutdown", numUser);
		shutdownId = shutdown.getId();
	}

	/**
	 * Initialises OpenCloud parameters. This method should be called before creating any entities.
	 * <p>
	 * Inside this method, it will create the following OpenCloud entities:
	 * <ul>
	 * <li>CloudInformationService.
	 * <li>OpenCloudShutdown
	 * </ul>
	 * <p>
	 * 
	 * @param numUser the number of User Entities created. This parameters indicates that
	 *            {@link OpenCloudShutdown} first waits for all user entities's
	 ...
	 * @param traceFlag <tt>true</tt> if OpenCloud trace need to be written
	 * @see OpenCloudShutdown
	...
	public static void init(int numUser, Calendar cal, boolean traceFlag) {
		try {
			...
			Log.println("OpenCloud.init(): The simulation has been terminated due to an unexpected error");
	...
	public static void init(int numUser, Calendar cal, boolean traceFlag, double minTimeBetweenEvents) {
	    ...
	    OpenCloud.minTimeBetweenEvents = minTimeBetweenEvents;
	}
	
	
	
	/**
	 * Starts the execution of OpenCloud simulation. It waits for complete execution of all entities,
	 ...
	 *             OpenCloud package or this entity name is <tt>null</tt> or empty.
	 * @see OpenCloud#init(int, Calendar, boolean)
	 * @pre $none
	 * @post $none
	 */
	public static double startSimulation() throws NullPointerException {
		Log.printlnConcat("Starting OpenCloud version ", OPENCLOUD_VERSION_STRING);
		try {
			double clock = run();
			...
			e.printStackTrace();
			throw new NullPointerException("OpenCloud.startSimulation() :"
					+ " Error - you haven't initialized OpenCloud.");
		}
	}
	...
	public static void stopSimulation() throws NullPointerException {
		try {
			...
			throw new NullPointerException("OpenCloud.stopSimulation() : "
	 ...
	 * 
	 * @return a new copy of Calendar object or if OpenCloud hasn't been initialized
	 * @pre $none
	 * @post $none
	 */
	...
	protected static void initialize() {
		...
		// Also reset the auto-id counters to make assignment reset on a new call to OpenCloud::init to mimic old behavior.
		Cloudlet.initialize();
		Host.initialize();
		Pe.initialize();
		Vm.initialize();
	...
	private static boolean runClockTick() {
		...
		if (future.isEmpty()) {
			running = false;
			printMessage(OpenCloud.clock()+": Simulation: No more future events");
			return false;
		}
	 ...
	 * @param data the data
	 */
	public static void send(int srcId, int dstId, double delay, OpenCloudTags tag, Object data) {
		if (delay < 0) {
	 ...
	 * @param tag the tag
	 */
	public static void sendFirst(int srcId, int dstId, double delay, OpenCloudTags tag, Object data) {
		if (delay < 0) {
			throw new IllegalArgumentException("Send delay can't be negative.");
		}
		if (delay >= Double.MAX_VALUE) {
