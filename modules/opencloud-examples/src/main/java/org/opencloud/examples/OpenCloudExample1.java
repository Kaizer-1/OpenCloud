 ...
 */
public class OpenCloudExample1 {
	...
	public static void main(String[] args) {
		Log.println("Starting OpenCloudExample1...");

		try {
			// First step: Initialize the OpenCloud package. It should be called before creating any entities.
			int num_user = 1; // number of cloud users
			Calendar calendar = Calendar.getInstance(); // Calendar whose fields have been initialized with the current date and time.
 			boolean trace_flag = false; // trace events

			/* Comment Start - Dinesh Bhagwat 
			 * Initialize the OpenCloud library. 
			 * init() invokes initCommonVariable() which in turn calls initialize() (all these 3 methods are defined in OpenCloud.java).
			 ...
			 * Once initialize() returns (note that we are in method initCommonVariable() now), a OpenCloudShutdown (which is derived from SimEntity) instance is created 
			 * (with numuser as 1, its name as OpenCloudShutdown, id as -1, and state as RUNNABLE). Then this new entity is added to the simulation 
			...
			// Datacenters are the resource providers in OpenCloud. We need at
			// list one of them to run a OpenCloud simulation
			...
			Log.println("OpenCloudExample1 finished!");
		} catch (Exception e) {
			e.printStackTrace();
