...
public class OpenCloudMultiExtensionPaperExampleRunner {
    ...
    public static void main(String[] args) {
        ...
        OpenCloudMultiExtensionPaperExample.vmVirtOverhead = 5;
        OpenCloudMultiExtensionPaperExample.contVirtOverhead = 3;
        ...
        // Runner loop
        String voString = (OpenCloudMultiExtensionPaperExample.vmVirtOverhead + OpenCloudMultiExtensionPaperExample.contVirtOverhead > 0) ? "VO" : "none";
        for (int activation : activations) {
            OpenCloudMultiExtensionPaperExample.numberOfPeriodicActivations = activation;
            String actString = (activation > 1) ? "many" : "single";

            for (var config : configs.entrySet()) {
                OpenCloudMultiExtensionPaperExample.cloudletToGuest = config.getValue();

                OpenCloudMultiExtensionPaperExample.numberOfContainers = (config.getKey().indexOf('V') >= 0) ? 0 : 4;
                OpenCloudMultiExtensionPaperExample.numberOfVms = (config.getKey().indexOf('C') >= 0) ? 0 : 4;

                // special nested virtualization scenario
                OpenCloudMultiExtensionPaperExample.nestedContainers = config.getKey().indexOf('N') >= 0;

                for (var p : payloads.entrySet()) {
                    System.out.println("Running '" + actString + "' example with payload " + p.getValue() + " and config " + config.getKey());

                    OpenCloudMultiExtensionPaperExample.payloadSize = p.getValue();
                    OpenCloudMultiExtensionPaperExample.fileInfo = "-" + p.getKey() + "-" + config.getKey() + "-" + actString + "-" + voString;

                    OpenCloudMultiExtensionPaperExample.main(emptyArgs);
                    System.out.println();
                }
            }
        }
